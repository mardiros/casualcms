...
const PageRow: React.FunctionComponent<PageRowProps> = (props: PageRowProps) => {
...
const PageListTable: React.FunctionComponent<PageListTableProps> = (props: PageListTableProps) => {
...
const PageListButtons: React.FunctionComponent<PageListButtonsProps> = (props: PageListButtonsProps) => {
  const curPage = props.curPage;
  let navigate = useNavigate();
  const qs = curPage ? new URLSearchParams({ parent: curPage.path, type: curPage.type }) : "";
  return <Stack p={4} spacing={4} direction='row' align='right'>
    <Button onClick={() => navigate(`/admin/page/new?${qs}`, { "replace": true })}>
